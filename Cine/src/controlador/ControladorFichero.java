...
public class ControladorFichero {
	...
	public boolean escribirCompra(String nombreFichero, ArrayList<Carrito> carrito, Cliente cliente, Compra compra) {
		...
		try {
			 ...
			 * existe.
			 */
			BufferedWriter escritor = new BufferedWriter(new FileWriter(ruta + nombreFichero, true));
			...
			// ===== TOTALES =====
			escritor.write("TOTAL SIN DESCUENTO: " + String.format("%.2f", compra.getPrecioTotal()) + " €");
			escritor.newLine();

			escritor.write("DESCUENTO APLICADO: -" + String.format("%.2f", compra.getDescuentoAplicado()) + " €");
			escritor.newLine();

			double totalFinal = compra.getPrecioTotal() - compra.getDescuentoAplicado();
			escritor.write("TOTAL A PAGAR: " + String.format("%.2f", totalFinal) + " €");
			escritor.newLine();
